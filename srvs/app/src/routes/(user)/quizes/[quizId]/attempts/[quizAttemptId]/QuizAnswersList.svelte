<script lang="ts">
	...
	let hasRated = $derived(item.rating !== 0);
			...
			<div class="mt-8 flex flex-col items-center justify-center gap-4">
				{#if !hasRated}
					<div class="flex flex-col items-center gap-2">
						<p class="text-base-content/60 text-sm font-medium">Is this question helpful?</p>
						<div class="flex items-center gap-4">
							<button
								class="btn btn-circle btn-ghost hover:bg-error/10 hover:text-error transition-colors"
								aria-label="Dislike"
							...
							<button
								class="btn btn-circle btn-ghost hover:bg-success/10 hover:text-success transition-colors"
								aria-label="Like"
								onclick={() => handleRate(1)}
							>
								<ThumbsUp size={24} />
				...
				{/if}
				{#if showThankYou}
					<div role="alert" class="alert alert-success w-auto flex-row px-4 py-2 text-sm shadow-sm">
						<span>Thanks for helping with quiz quality!</span>
					</div>
				{/if}
			</div>
		{/if}
