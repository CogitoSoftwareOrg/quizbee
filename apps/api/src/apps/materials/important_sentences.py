...
def summarize_to_fixed_tokens(
    text: str, target_token_count: int, summary_token_count: int, num_chunks: int = 10
) -> tuple[str, str]:
    """
    ...
    if total_tokens_in_doc < target_token_count:
        print(
            "Предупреждение: Весь документ меньше целевого размера. Возвращается полный текст."
        )
        return (text, text)
    ...
    if brief_token_count > summary_token_count:
        print(
            f"Краткое резюме ({brief_token_count} токенов) превышает лимит ({summary_token_count}). Отбираем топ-абзацы..."
        )
        # Пересортируем абзацы по score и отбираем пока не достигнем лимита
        scored_paragraphs = [
            (score_paragraph(paragraphs[idx]), idx, para_lengths[idx])
    ...
    print(
        f"Краткое резюме готово: {brief_token_count} токенов (цель: {summary_token_count})."
    )
    ...
    for priority, idx, length in candidates:
        if idx not in extended_indices:
            if (
                current_token_count + length <= target_token_count * 1.1
            ):  # Допускаем 10% превышение
    ...
    print(
        f"Шаг 5: Сборка основного конспекта... Выбрано {len(extended_indices)} абзацев (включая контекст)."
    )
    ...
    print(
        f"Финальный размер основного конспекта: {final_token_count} токенов (цель: {target_token_count})."
    )

    # Если всё ещё сильно превышаем лимит, обрезаем контекстные параграфы
    if final_token_count > target_token_count * 1.15:
        print(
            f"Превышение лимита более чем на 15%. Удаляем лишние контекстные параграфы..."
        )
